<template>
    <div id="home-main">
        <v-card id="month-card">
            <v-card-title>Usage of drugs by months in this year ({{new Date().getFullYear()}})</v-card-title>
            <v-sparkline
        ...
        <v-card id="year-card">
            <v-card-title>Usage of drugs by years (previous ({{new Date().getFullYear() - 1}}), this ({{new Date().getFullYear()}}))</v-card-title>
            <v-sparkline
...
<script>
    export default {
        ...
        methods:{
                ...
                this.refreshToken().then(response => {
                    ...
                    this.axios.get("api/pharmacyadmin", {headers:{"Authorization": "Bearer " + localStorage.jws, "Content-Type" : "application/json",}})
                        .then(response => {
                            ...
                            this.getCountDrugsByMonth();
                            this.getCountDrugsByYear();
                        })
                        .catch(response => {
            ...
            getCountDrugsByMonth: function() {
                this.axios.get("api/pharmacyadmin/pharmacydrugmonth/" + this.pharmacyId)
                    .then(response =>
                    {
            ...
            getCountDrugsByYear: function() {
                this.axios.get("api/pharmacyadmin/pharmacydrugyear/" + this.pharmacyId)
                    .then(response =>
                    {
