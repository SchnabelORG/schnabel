<template>
    <div id="home-main">
        <v-card id="month-card">
            <v-card-title>Income by months in this year ({{new Date().getFullYear()}})</v-card-title>
            <v-sparkline
        ...
        <v-card id="year-card">
            <v-card-title>Income by years (previous ({{new Date().getFullYear() - 1}}), this ({{new Date().getFullYear()}}))</v-card-title>
            <v-sparkline
    ...
    export default {
        ...
        methods:{
            ...
            getIncomeByMonth: function() {
                this.axios.get("api/appointment/pharmacyincomemonth/" + this.pharmacyId)
                    .then(response =>
                    {
            ...
            getIncomeByYear: function() {
                this.axios.get("api/appointment/pharmacyincomeyear/" + this.pharmacyId)
                    .then(response =>
                    {
